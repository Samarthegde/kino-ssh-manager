...
- **Fleet update check** - asks each host what packages it has waiting and which of those are security updates, read from the host's own package manager (apt, dnf, yum, zypper, apk, pacman) rather than any third-party vulnerability feed. Where a package manager can't separate security updates it says so instead of reporting zero, and a host that couldn't be reached stays in the list with its reason. *Apply* opens a terminal with the upgrade command ready, unrun. Lives under **Settings → Security → Updates**.
